...
namespace OXXO.Controllers
{
    public class PermisosController : Controller
    {
        ...
        public JsonResult GetPermisos(string IdPerfil, string IdControlador, string IdAccion)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(dbConn))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("SELECT rl.Leer, rl.Crear, rl.Editar, p.edicion, p.carga, p.aprobacion, p.categorizacion FROM RolControlador as rl INNER JOIN Perfil as p ON p.IdPerfil = rl.IdPerfil WHERE p.IdPerfil=" + IdPerfil + " AND IdControlador=" + IdControlador + " AND IdAccion=" + IdAccion, connection))
                    {
                        SqlDataReader dr = command.ExecuteReader();
        ...
        public PermisosUsuario GetPermisosUsuario(string NombreAccion, string NombreControlador, string IdPerfil) 
        {
                    ...
                    using (SqlConnection connection = new SqlConnection(dbConn))
                    {
                        ...
                        using (SqlCommand command = new SqlCommand("SP_BuscaPermisoUsuario", connection))
                        {
                            ...
                            while (dr.Read())
                            {
                                ...
                                if (userPermiso.Crear && userPermiso.Editar && userPermiso.Leer)
                                {
                                    userPermiso.edicion = Convert.ToBoolean(dr["edicion"]);
                                    userPermiso.carga = Convert.ToBoolean(dr["carga"]);
            ...
            {
                ViewBag.Alert = CommonServices.ShowAlert(Alerts.Danger, "Error: " + ex.ToString());
                throw;
            }
